...
open class GenericMongoDAO<T>(entityType: Class<T>) {
    ...
    //recibe un bson con la condicion de busqueda y otro bson donde
    //se pasa el atributo por el cual ordenar ej: "{fecha:1}" orden ascendente
    fun findSort(filter: Bson,atribSort:Bson):List<T>{
        if(connection.session != null) {
            return collection.find(connection.session!!, filter).sort(atribSort).into(mutableListOf())
        }
        return collection.find(filter).sort(atribSort).into(mutableListOf())
    }
